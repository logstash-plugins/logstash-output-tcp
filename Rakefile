...
task :gen_certs do
  ...
  `openssl req -new -x509 -key #{plaintext_path}/instance.key -out #{plaintext_path}/instance.crt -days 36500 -subj "/CN=localhost"`
  `openssl req -new -x509 -key #{plaintext_path}/instance.key -out #{encrypted_path}/instance.crt -days 36500 -subj "/CN=localhost"`
end
